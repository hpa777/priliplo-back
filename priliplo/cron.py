    ...
    def do(self):
        ...
        for arr in need_dates:
            ...
            for application in campaign_application:
                send_user_notification(user=application.client, payload={"head": "Фотоотчеты", "body": "Вам необходимо отправить фотоотчет в сервисе priliplo.ru!", "url": "https://priliplo.ru"}, ttl=1000)
    ...
    def do(self):
        ...
        for campaign in campaigns:
            ...
            if today == (campaign.approve_date + campaign_billing_period_1) or today == (campaign.approve_date + campaign_billing_period_2) or today == (campaign.approve_date + campaign_billing_period_3):
                ...
                # сколько автомобилей реально участвует
                active_applications = CampaignApplication.objects.filter(campaign=campaign).filter(application_status=2).count()

                new_campaign_balance = advertiser_settings.period_cost * active_applications
