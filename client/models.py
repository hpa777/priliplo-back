...
from company.models import Campaign, Advertiser, CampaignBalanceHistory, AdvertiserSettings
...
class UserProfile(models.Model):
    ...
    rsa_info = models.TextField(_("Информация из RSA"), blank=True, null=True)
    gibdd_info = models.TextField(_("Информация из ГИБДД"), blank=True, null=True)
...
class CampaignApplication(models.Model):  # заявка на участие в акции
    ...
    def check_for_need_photoreport(self, *args, **kwargs):
        application_date = self.approve_date
        photoreport_start_date = application_date.strftime("%Y-%m-%d")
        today = datetime.now(tz=get_current_timezone())
        days_from_accept = (today.date() - application_date).days

        start_date = 0
        end_date = 0
        current_period = 0

        if days_from_accept >= 14 and days_from_accept < 28:
            start_date = 14
            end_date = 28
            current_period = 2
        elif days_from_accept >= 28 and days_from_accept < 42:
            start_date = 28
            end_date = 42
            current_period = 3
        elif days_from_accept >= 42 and days_from_accept < 56:
            start_date = 42
            end_date = 56
            current_period = 4
        elif days_from_accept >= 56 and days_from_accept < 70:
            start_date = 56
            end_date = 70
            current_period = 5
        elif days_from_accept >= 70 and days_from_accept < 84:
            start_date = 70
            end_date = 84
            current_period = 6
        elif days_from_accept >= 84 and days_from_accept < 98:
            start_date = 84
            end_date = 98
            current_period = 7
        ...
        if start_date != 0 and end_date != 0:
            ...
            photoreports = PhotoReport.objects.filter(Q(photoreport_status=1) | Q(photoreport_status=2)).filter(edited_date__gte=edit_date_start_date).filter(edited_date__lt=edit_date_end_date).filter(application=self).first()
            if photoreports:
        ...
        if 'return_tuple' in kwargs:
            return (need_photoreport, photoreport_start_date, days_from_accept, current_period)
        else:
...
class PhotoReport(models.Model):  # фотоотчет
    ...
    def accept(self):
        ...
        # campaign_balance_history_object = CampaignBalanceHistory.objects.create(advertiser=self.advertiser, campaign=self.campaign, balance_decrease=advertiser_settings.period_cost, change_type=5)
