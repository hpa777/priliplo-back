...
class CampaignForm(forms.ModelForm):
    ...
    def clean_quota(self):
        ...
        quota = int(''.join(i for i in str(quota) if i.isdigit()))
        return quota

    def clean_award(self):
        award = self.cleaned_data.get("award")
        # parse digits from the string
        award = int(''.join(i for i in str(award) if i.isdigit()))
        return award
...
class CampaignBalanceHistory (models.Model):
    CHANGE_TYPE_CHOICES = (
        ...
        (5, _("Списание за одобренный фотоотчет")),
        (6, _("Списание за подготовку логотипа")),
        (7, _("Списание за оклейку автомобиля")),
    )
